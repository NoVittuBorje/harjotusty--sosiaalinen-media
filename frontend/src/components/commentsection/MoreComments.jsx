...
const MoreComments = ({comment, User,postid }) => {
    const { data, error, loading,refetch } = useGetComments({commentid:comment.id})
    ...
    const handleReply = ({content,commentid}) => {
        console.log(content)
        const data = newcomment({postid:postid,content:content,replyto:commentid})
        console.log(data)
      };
        ...
        <List>
          {morecomments.replies.map((reply) => (
            <ListItem key={reply.id}>
              <Comment comment={reply} User={User} handleReply={handleReply} handleDelete={handleDelete} handleModify={handleModify}/>
            </ListItem>
          ))}
        </List>
    )}else{
